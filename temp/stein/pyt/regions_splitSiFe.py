...
# ===== now Fe ----
    ...
    outfile='outmatrix/spectrumresults_'+region+'_Fe.txt'
    if (writeFiles):
       print "Connecting to DB server...."
       conn = mysql.connector.connect(host="127.0.0.1", port=3309, user="seh", db="Cluster")    
       cursor = conn.cursor()
       sqlfile = '../_matrx_spectrum_Fe.sql'
       fd = open(sqlfile, 'r') 
       query = fd.read()
       query=query.replace("@mom","avg");
       query=query.replace("@err","stddev");
...
# -- sepcial treatment for lobe:two separate regions
	 ...
	 query=query.replace("#1","")
       ...
       with open(outfile, 'wb') as fp:
          pickle.dump(result, fp)
       print "Writing Fe SPECTRA to file..."
    else:
       with open (outfile, 'rb') as fp:
          result = pickle.load(fp) 
       print "Reading Fe SPECTRA from file (!NB No DB connection)"
...
# -- GSE in SW, GSM else ---
 ...
 # plot settings
    ...
    spectrum.errorbar(FeEnergy,FeFlux, marker='o',color='green',markersize=20,linewidth=2)
...
#    spectrum.text(0.95,ytextpos-2*deltaytext,'Date = ['+str(minDate)+" - "+ str(maxDate)+"]",ha='right',transform=spectrum.transAxes)
    spectrum.text(0.95,ytextpos-deltaytext,ptit,ha='right', transform=spectrum.transAxes)
    spectrum.text(0.95,ytextpos-2*deltaytext,'Si, slope = '+"%.1f" % SiSlope+' ('+str(numSi)+' samples)',color='blue',ha='right',transform=spectrum.transAxes)
    spectrum.text(0.95,ytextpos-3*deltaytext,'Fe, slope = '+"%.1f" % FeSlope+' ('+str(numFe)+' samples)',color='green',ha='right',transform=spectrum.transAxes)#    E_TOF.text(xtextpos,ytextpos-3*deltaytext,'Ygse = ['+str(minY)+" - "+ str(maxY)+"]",transform=E_TOF.transAxes)
