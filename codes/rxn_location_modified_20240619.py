...
marker_size = 10
ax[0].scatter(
    df_shear_pos_z.spc_pos_y,
    df_shear_pos_z.spc_pos_x,
    marker="o",
    color=color_list[1],
    alpha=0.5,
    s=marker_size,
    # label=r"$B_{\rm Z} > 0$",
    facecolors=color_list[1],
...
ax[0].scatter(
    df_shear_neg_z.spc_pos_y,
    df_shear_neg_z.spc_pos_x,
    marker="o",
    color=color_list[2],
    s=marker_size,
    # label=r"$B_{\rm Z} < 0$",
    facecolors=color_list[2],
    edgecolors=color_list[2],
    alpha=0.5,
)
ax[0].set_xlabel(r"Y [GSM, $R_{\rm e}$]", fontsize=label_size)
ax[0].set_ylabel(r"X [GSM, $R_{\rm e}$]", fontsize=label_size)
...
ax[0].annotate(
    ...
    xy=(-16.4, 1),
    xytext=(-5, -3.5),
    arrowprops=dict(arrowstyle="->", color=color_list[3], lw=2),
    fontsize=label_size,
    color=color_list[3],
)
...
ax[0].text(
    13, -4, "Dusk", fontsize=label_size, color=color_list[-1], ha="left", va="center"
)
ax[0].text(
    -13, -4, "Dawn", fontsize=label_size, color=color_list[0], ha="right", va="center"
)
...
ax[1].set_xlabel(r"Y [GSM, $R_{\rm e}$]", fontsize=label_size)
ax[1].set_ylabel(r"Z [GSM, $R_{\rm e}$]", fontsize=label_size)
...
# Write dawn and dusk labels
ax[1].text(
    7, -8.5, "Dusk", fontsize=label_size, color=color_list[-1], ha="left", va="center"
)
ax[1].text(
    -7, -8.5, "Dawn", fontsize=label_size, color=color_list[0], ha="right", va="center"
)
...
plt.savefig(
    "../figures/mms_rxn_location_modified.pdf",
    dpi=300,
    bbox_inches="tight",
    transparent=True,
    pad_inches=0.05,
