...
ax[0].set_xlabel(r'Y [$R_\oplus$]', fontsize=label_size * 1.2)
ax[0].set_ylabel(r'X [$R_\oplus$]', fontsize=label_size * 1.2)
...
# Make a function to give a parameterized parabola using the equation y = a(x-h)^2 + k
'''
def parabola(a, h, k, x):
    return a*(x-h)**2 + k

msp_x = np.linspace(-25, 25, 1000)
msp_y = parabola(-0.05, 0, 10, msp_x)

msh_x = np.linspace(-25, 25, 1000)
msh_y = parabola(-0.05, 0, 0, msh_x)

ax[0].plot(msp_x, msp_y, color='k', linestyle='--', label='parabola')
'''


'''
# Getting X and Y from Shue et al. 1998
R_E = 6371.2
theta = np.linspace(0, 2*np.pi, 1000)
x_shu = (10.22 + 1.29 * np.sin(theta/2)**2)/(1 + 0.23 * np.sin(theta/2)**2) * (10/R_E)
D = 6.6 + 4 * np.sin(theta/2)**2 * 10/R_E
R = D / np.cos(theta/2) * (10/R_E)

# Convert magnetopause position from polar to cartesian coordinates
x_shu = R * np.cos(theta)
y_shu = R * np.sin(theta)

ax[0].plot(y_shu, x_shu, color='b', linestyle='--', label='Shue et al. 1998')
'''
...
ax[0].annotate("Magnetopause", xy=(-16.4, 1), xytext=(0, -3), arrowprops=dict(arrowstyle="->", 
            color=color_list[3], lw=2), fontsize=label_size, color=color_list[3])

# Write dawn and dusk labels
ax[0].text(20, -4, 'Dusk', fontsize=label_size, color=color_list[0], ha='left', va='center')
ax[0].text(-20, -4, 'Dawn', fontsize=label_size, color=color_list[0], ha='right', va='center')

# Write panel (a) at the top right corner of the plot
ax[0].text(0.1, 0.95, '(a)', fontsize=label_size, color='k', ha='right', va='top',
...
ax[1].set_xlabel(r'Y [$R_\oplus$]', fontsize=label_size * 1.2)
ax[1].set_ylabel(r'Z [$R_\oplus$]', fontsize=label_size * 1.2)
...
# Write dawn and dusk labels
ax[1].text(14, -10, 'Dusk', fontsize=label_size, color=color_list[0], ha='left', va='center')
ax[1].text(-14, -10, 'Dawn', fontsize=label_size, color=color_list[0], ha='right', va='center')

# Write panel (b) at the top left corner of the plot
ax[1].text(0.1, 0.95, '(b)', fontsize=label_size, color='k', ha='right', va='top',
