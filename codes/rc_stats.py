...
fnames = np.sort(glob.glob(f"{data_folder}/reconnection_line_data_mms3_v2_20220926.csv"))
...
for file_name in fnames:
    for cut_type in cut_type_list[:]:
        ...
        fig_inputs ={
            'bins': np.logspace(0, np.log10(15), 15),
            'file_name': file_name,
            'dark_mode': False,
            'fig_name':  f"rx_hist_{mms_probe_num}",
            'fig_format': 'jpg',
            'fig_folder': '../figures/rx_hist/rx_hist_v08',
            'fig_size': (8, 8),
            'histtype': 'step',
            'linewidth': 3,
            'cut_type': cut_type,
