...
y_arr = np.linspace(0, 120, 121)
z_arr = np.linspace(0, 120, 121)
...
first_deri=(Lxx+Lyy)/2
second_deri=np.sqrt((Lxx+Lyy)**2- 4*(Lxx*Lyy-Lxy**2))/2

lambda_1 = first_deri + second_deri
lambda_2 = first_deri - second_deri
lambda_0 = np.concatenate((lambda_1, lambda_2), axis=0)

lambda_max_idx = np.argmax(abs(lambda_0), axis=0)
lambda_min_idx = np.argmin(abs(lambda_0), axis=0)

lambda_max = np.full(121, np.nan)
lambda_min = np.full(121, np.nan)

for j,xx in enumerate(lambda_max_idx):
    if(lambda_0[xx,j] <0):
        lambda_max[i] = lambda_0[xx, j]

for j,xx in enumerate(lambda_min_idx):
    if(lambda_0[xx,j] <0):
        lambda_min[i] = lambda_0[xx, j]


#lambda_max = np.amax(abs(lambda_0), axis=0)
#lambda_min = np.amin(abs(lambda_0), axis=0)
...
y_shu_arr = np.linspace(y_shu_min, y_shu_max, np.shape(y_shu)[0])
z_shu_arr = np.linspace(z_shu_min, z_shu_max, np.shape(z_shu)[0])
...
axs1.plot(y_shu_arr, z_shu_arr[idx], 'kd', ms=1, lw=2)
