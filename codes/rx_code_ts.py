...
for center_time_str in center_time_list[ind_min:ind_max]:
    ...
    start_time = (center_time - pd.Timedelta("30 minute")).tz_convert("UTC")
    end_time = (center_time + pd.Timedelta("30 minute")).tz_convert("UTC")
    ...
    for ind_range, trange in enumerate(
        trange_list[ind_t_min:ind_t_max:1], start=ind_t_min
    ):
        trange_min = trange - pd.Timedelta("0.5 minute")
        trange_max = trange + pd.Timedelta("0.5 minute")
        trange = [
            trange_min.strftime("%Y-%m-%d %H:%M:%S"),
            trange_max.strftime("%Y-%m-%d %H:%M:%S"),
        ]
        ...
        for foo in range(1):
            ...
            try:
                ...
                shear_norm = (shear - np.nanmin(shear)) / (
                    np.nanmax(shear) - np.nanmin(shear)
                )
                rx_en_norm = (rx_en - np.nanmin(rx_en)) / (
                    np.nanmax(rx_en) - np.nanmin(rx_en)
                )
                va_cs_norm = (va_cs - np.nanmin(va_cs)) / (
                    np.nanmax(va_cs) - np.nanmin(va_cs)
                )
